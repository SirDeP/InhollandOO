...
namespace ProjectRobotica.Controller
{
    internal class PS4Controller
    {
        ...
        public async Task<bool> UpdateAsync()
        {
            ...
            try
            {
                // Read a packet of data from the stream asynchronously
                int bytesRead = await ControllerStream.ReadAsync(buffer, 0, packetSize);
            ...
            {
                if (debug) Console.WriteLine("Unknown event occurred (this is normal immediately after connection)");
            }
